...
The we load the `txt` files includes in `rexposome` package so we can load the exposures and see the amoung of missing data (check vignette *Exposome Data Analysis* for more information). 
...
Once this is done, the *class* of each column needs to be set, so `mice` will be ablte to differenciate between continuous and categorical exposures.

```{r class_imputation}
for(ii in c(1:54, 59:88, 95:96)) {
    dta[, ii] <- as.numeric(dta[ , ii])
}
for(ii in c(55:58, 89:94)) {
    dta[ , ii] <- as.factor(dta[ , ii])
}
```

With this `data.frame` we perform the imputation calling `mice` functions (for more information about this call, check `mice`'s vignette). We remove the columns *birthdate* in order sicne it is not necessary for the imputations and carries lots of categories.

```{r mice_imputation, message=FALSE}
imp <- mice(dta[ , -93], pred = quickpred(dta[ , -93], mincor = 0.2, minpuc = 0.4),
